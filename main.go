// Command fresnel-tm is a thin-film characteristic-matrix calculator. It
// takes a coating stack (incident medium, layers, substrate), a wavelength
// and an angle of incidence, and reports reflectance, transmittance and
// absorptance. A spectrum mode sweeps R(λ)/T(λ)/A(λ) over a wavelength band.
//
// Usage:
//
//	go run . -http :8080                 # serve the web console + /api on :8080
//	go run . -solve example/ar-quarter.json   # CLI solve of a packaged example
//
// The web console (embedded in the binary) loads packaged examples, posts
// them to /api/stack and /api/spectrum, and plots the returned numbers. The
// -solve flag is the same solver with no HTTP server, useful in scripts.
package main
...
// assets carries the web console and the packaged examples inside the binary
// so the tool runs from any working directory (e.g. inside a container).
//
...
// runSolve loads an example file, validates it, solves it at the design
// wavelength and prints a short report. Any failure goes to stderr with a
// non-zero exit code.
