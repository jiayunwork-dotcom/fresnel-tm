...
// TestValidateMaterialRejectsBadIndex checks the material invariants: a
// non-positive index and a negative extinction coefficient must both error.
...
// TestValidateLayerRejectsNegativeThickness is the simple surface: a coating
// with a negative thickness is rejected before any physics runs.
...
// TestValidateIncidenceRejectsBadValues checks the incidence invariants:
// wavelength ≤ 0, angle ≥ 90° and negative angle are all errors.
...
// TestParsePolarization checks accepted spellings and rejects typos.
...
// TestExampleARQuarterLoads verifies the packaged example file: it parses,
// validates, and carries the expected quarter-wave geometry.
...
// TestStackZeroLayersIsLegal checks that an empty layer list validates: a
// bare incident/substrate interface is a legal problem (the Fresnel limit).
