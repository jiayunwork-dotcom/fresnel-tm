...
func SolveStackRequest(req *model.StackRequest) (model.StackResult, error) {
	if req == nil {
		return model.StackResult{}, fmt.Errorf("求解失败：空请求")
	}
	return SolveRequest(req, req.Incidence)
}

func SolveRequest(req *model.StackRequest, snap model.Incidence) (model.StackResult, error) {
	if req == nil {
		return model.StackResult{}, fmt.Errorf("求解失败：空请求")
	}
	...
	use := snap
	if req.AngleDeg != snap.AngleDeg || req.WavelengthNm != snap.WavelengthNm {
		use = req.Incidence
	}
	return Solve(req.Stack(), use, pol)
}
